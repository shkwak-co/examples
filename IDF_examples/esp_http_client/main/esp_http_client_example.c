...
static void
http_rest_with_url (void)
{
  ESP_LOGE (TAG, "REST_WITI_URL");
...
{
  ESP_LOGE (TAG, "REST_WITH_HOSTNAME_PATH");
  esp_http_client_config_t config = {
    .host = CONFIG_EXAMPLE_HTTP_ENDPOINT,
    .path = "/get",
...
static void
https_with_hostname_path (void)
{
  ESP_LOGE (TAG, "HTTPS_WITI_HOSTNAME_PATH");
  esp_http_client_config_t config = {
...
static void
http_encoded_query (void)
{
  ESP_LOGE (TAG, "ENCODED_QUERY");
  esp_http_client_config_t config = {
    .host = CONFIG_EXAMPLE_HTTP_ENDPOINT,
    .path = "/get",
    .event_handler = _http_event_handler,
...
static void
http_relative_redirect (void)
{
  ESP_LOGE (TAG, "RELATIVE_REDIRECT");
  esp_http_client_config_t config = {
...
static void
http_absolute_redirect (void)
{
  ESP_LOGE (TAG, "ABSOLUTE_REDIRECT");
  esp_http_client_config_t config = {
...
static void
http_absolute_redirect_manual (void)
{
  ESP_LOGE (TAG, "ABSOLUTE_REDIRECT_MANUAL");
  esp_http_client_config_t config = {
    .url = "http://" CONFIG_EXAMPLE_HTTP_ENDPOINT "/absolute-redirect/3",
    .event_handler = _http_event_handler,
    .disable_auto_redirect = true,
...
static void
http_redirect_to_https (void)
{
  ESP_LOGE (TAG, "REDIRECT_TO_HTTPS");
  esp_http_client_config_t config = {
...
static void
http_download_chunk (void)
{
  ESP_LOGE (TAG, "DOWNLOAD_CHUNK");
  esp_http_client_config_t config = {
...
static void
http_perform_as_stream_reader (void)
{
  ESP_LOGE (TAG, "PERFORM_AS_STREAM_READER");
...
https_async (void)
{
  ...
  const char *post_data =
    "Using a Palantír requires a person with great strength of will and wisdom. The Palantíri were meant to "
...
static void
https_with_invalid_url (void)
{
  ESP_LOGE (TAG, "HTTPS_WITH_INVALID_URL");
  esp_http_client_config_t config = {
 ...
 */
static void
http_native_request (void)
{
  ESP_LOGE (TAG, "NATIVE_REQUEST");
  // Declare local_response_buffer with size (MAX_HTTP_OUTPUT_BUFFER + 1) to prevent out of bound access when
  // it is used by functions like strlen(). The buffer should only be used upto size MAX_HTTP_OUTPUT_BUFFER
...
#if CONFIG_MBEDTLS_CERTIFICATE_BUNDLE
static void
http_partial_download (void)
{
  ESP_LOGE (TAG, "PARTIAL_DOWNLOAD");
  esp_http_client_config_t config = {
...
static void
http_test_task (void *pvParameters)
{
  http_rest_with_url ();
