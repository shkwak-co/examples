...
static void
query_mdns_service (const char *service_name, const char *proto)
{
...
#if CONFIG_MDNS_PUBLISH_DELEGATE_HOST
static void
lookup_mdns_delegated_service (const char *service_name, const char *proto)
{
...
static bool
check_and_print_result (mdns_search_once_t * search)
{
...
static void
query_mdns_hosts_async (const char *host_name)
{
  ESP_LOGI (TAG, "Query both A and AAAA: %s.local", host_name);
...
static void
query_mdns_host (const char *host_name)
{
  ...
  if (!new_level && old_level)
    {
      ...
      //query_mdns_service("_computer", "_tcp");
...
static void
mdns_example_task (void *pvParameters)
{
#if CONFIG_MDNS_RESOLVE_TEST_SERVICES == 1
...
app_main (void)
{
  ...
  ESP_ERROR_CHECK (mdns_netif_action
                   (EXAMPLE_INTERFACE, MDNS_EVENT_IP4_REVERSE_LOOKUP | MDNS_EVENT_IP6_REVERSE_LOOKUP));
#endif
 ...
 */
static void
query_mdns_host_with_gethostbyname (char *host)
{
 ...
 */
static void
query_mdns_host_with_getaddrinfo (char *host)
{
