...
basic_auth_get_handler (httpd_req_t * req)
{
  ...
  if (buf_len > 1)
      ...
      if (strncmp (auth_credentials, buf, buf_len))
        {
          ...
          httpd_resp_set_hdr (req, "WWW-Authenticate", "Basic realm=\"Hello\""); //<< 해당 코드가 로그인 프롬포트 생성
      ...
      else
        {
          ...
          // 관리 페이지에 basic_auth_info->username을 검사하여 맞으면 관리자 아니면 세션 종료 후 로그인 페이지로 이동?
          if (rc < 0)
            {
              ESP_LOGE (TAG, "asprintf() returned: %d", rc);
          ...
          httpd_register_uri_handler (req->handle, &getValue);
        }
      free (auth_credentials);
      free (buf);
    }
  else
    {
      ...
      httpd_resp_set_type (req, "application/json");
      httpd_resp_set_hdr (req, "Connection", "keep-alive");
      httpd_resp_set_hdr (req, "WWW-Authenticate", "Basic realm=\"Hello\"");  //<< 해당 코드가 로그인 프롬프트 출력
      httpd_resp_send (req, NULL, 0);
      
      httpd_unregister_uri (req->handle, "/getValue");
    }
...
hello_get_handler (httpd_req_t * req)
{
  ...
  buf_len = httpd_req_get_hdr_value_len (req, "Custom-Header-1") + 1;
  if (buf_len > 1)
    {
      buf = malloc (buf_len);
      if (httpd_req_get_hdr_value_str (req, "Custom-Header-1", buf, buf_len) == ESP_OK)
        {
          ESP_LOGI (TAG, "Found header => Custom-Header-1: %s", buf);
        }
      free (buf);
    }
  ...
  //static volatile int seq1 = 0;
  //httpd_resp_set_hdr (req, "Custom-Header-2", seq1++);
