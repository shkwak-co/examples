...
static void
print_peer_cert_info (const mbedtls_ssl_context * ssl)
{
...
static esp_err_t
test_get_handler (httpd_req_t * req)
{
  httpd_resp_set_type (req, "text/html");
  httpd_resp_send (req, "<h1>test!</h1>", HTTPD_RESP_USE_STRLEN);

  return ESP_OK;
}


static const httpd_uri_t tested = {
  .uri = "/test",
  .method = HTTP_GET,
...
start_webserver (void)
{
  ...
  httpd_register_uri_handler (server, &tested);
  return server;
}
