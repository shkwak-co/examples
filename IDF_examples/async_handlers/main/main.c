...
static bool
is_on_async_worker_thread (void)
{
...
static esp_err_t
submit_async_req (httpd_req_t * req, httpd_req_handler_t handler)
{
...
static esp_err_t
long_async_handler (httpd_req_t * req)
{
...
static void
start_async_req_workers (void)
{

  // counting semaphore keeps track of available workers
  worker_ready_count = xSemaphoreCreateCounting (CONFIG_EXAMPLE_MAX_ASYNC_REQUESTS, // Max Count
                                                 0);  // Initial Count
...
{
  ESP_LOGI (TAG, "uri: /");
  const char *html = "<div><a href=\"/long\">long</a></div>" "<div><a href=\"/quick\">quick</a></div>";
